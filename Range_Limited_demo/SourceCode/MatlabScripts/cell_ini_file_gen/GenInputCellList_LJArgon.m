...
GEN_CELL_RANGE_X = [1 2 3 4];
GEN_CELL_RANGE_Y = [1 2 3 4];
GEN_CELL_RANGE_Z = [1 2 3 4];
...
tmp_pos_y = single(0);
tmp_pos_z = single(0);
command_file = 'mif_to_hex_command.tcl';
cmd = fopen(command_file, 'wt');
for x_ptr = 1:size(GEN_CELL_RANGE_X,2)
    ...
    for y_ptr = 1:size(GEN_CELL_RANGE_Y,2)
        ...
        for z_ptr = 1:size(GEN_CELL_RANGE_Z,2)
            ...
            % Generate the initialization file name (cell_ini_file_1_2_3.mif)
            cell_ini_file_name_wo_type = strcat('cell_ini_file_',num2str(cell_id_x),'_',num2str(cell_id_y),'_',num2str(cell_id_z));
            ...
            fprintf(cmd, strcat('qexec {mif2hex'," ",cell_ini_file_name_wo_type,'.mif'," ",cell_ini_file_name_wo_type,'.hex}\n'));
